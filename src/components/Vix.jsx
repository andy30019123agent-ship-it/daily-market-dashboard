...
export default function Vix({ vix, label }) {
  if (!vix) return null
  const chg = Number.isFinite(vix.change) ? vix.change : null
  const d = chg == null ? 'flat' : chg <= 0 ? 'down' : 'up'
  const g = Math.max(0, Math.min(1, Number.isFinite(vix.gauge) ? vix.gauge : 0.5))
  const pos = g * 100
  // 情緒位置（gauge 0=恐慌、1=貪婪）；VIX 越高→越恐慌
  const zone = g <= 0.33 ? { t: '偏恐慌', c: 'down' } : g <= 0.66 ? { t: '中性', c: 'neu' } : { t: '偏貪婪', c: 'up' }
  return (
    <section className="card" data-region="② 市場情緒 VIX">
      <div className="card-h"><span className="badge-pill label"><Activity size={14} strokeWidth={1.75} />{label} · VIX</span></div>
      <div className="vixbox">
        ...
        <div className={'vix-state mono ' + d}>
          {chg != null && <>{chg <= 0 ? '▼' : '▲'} {Math.abs(chg)}　</>}{vix.state}
        </div>
