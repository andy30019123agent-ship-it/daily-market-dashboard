...
- 狀態：approach 已 Andy 確認（做 B、題材不進回測、類股要進、資料源交我評估）；待 spec 審閱
- 前置：延續 `2026-07-04-radar-and-potential-upgrade-design.md`（發動信心分數）。本 spec 新增「回測調權重」離線研究工具，並為評分新增「類股強度分」子分。
...
- **題材分不進回測**：AI 當下上網查的發酵點無法回溯到過去某天重算，硬湊會有「用未來資訊作弊」的偏誤。題材維持固定加分，其權重由判斷決定、不被回測調整。
...
## 新增評分子分：類股強度分（sector_s）

Andy 要求類股納入評分並進回測。低基期股若身處「正在被法人吸籌／轉強」的類股，勝率較高。

- **定義**：個股所屬類股的整體法人淨買超強度（正規化 0~1）。用「該類股所有成分股的法人淨買超金額加總」相對全市場類股排名。
- **可回溯**：類股強度＝成分股法人資料的加總，完全由客觀資料重建，故**可進回測**。
- **線上端**：`potential.py` 新增 `sector_s` 子分並掛進 `build_potential`／`finalize_scores`；`DEFAULTS` 加 `w_sector`。權重先給暫定值（如各項均分），待回測報告出爐由 Andy 定案調整。
- 五項子分中，**回測調整籌碼／價量結構／基本面／類股 4 項**；題材固定。

## 資料源（混用，全免費、皆快取）

| 資料 | 來源 | 取法 | 備註 |
|---|---|---|---|
| 三大法人買賣超（籌碼、類股） | **TWSE 官方 T86**（`www.twse.com.tw/rwd/zh/fund/T86?date=YYYYMMDD&selectType=ALLBUT0999`） | 每交易日一次、回全市場所有個股 | 權威、對回測高效；上櫃另走 TPEX（spike 先做上市，上櫃列為延伸） |
| 歷史日 K（價量、報酬） | **FinMind** `TaiwanStockPrice` | 每檔一次回整段歷史 | 省工；上市+上櫃通用 |
| 月營收（基本面） | **FinMind** `TaiwanStockMonthRevenue` | 每檔一次 | 沿用線上既有 |
| 產業別歸屬（類股） | TWSE OpenAPI `t187ap03_L` | 一次回全市場產業別 | 沿用線上 `build_sector_constituents` |
...
1. **重建候選（每個 as-of 日期）**
   - 選一組回測基準日（例如視窗內每個交易日、或每週一次以省呼叫）。
   - 對每個基準日 D：用 D 往前 window 天的 T86 逐日資料，聚合各股法人淨買超＋買超天數 → 篩「近 N 日法人吸籌」候選（同線上 `pick_accumulators`）。
   - 對候選查 FinMind 一年日 K（截止 D，**不得用 D 之後資料**）算 price_pos/vol_ratio/above_ma60 → struct_s、低基期 gate；查月營收（截止 D）→ fund_s；用 T86＋產業別聚合類股強度 → sector_s。
   - 產出每個候選的四子分（複用 `potential.py` 的 `chip_score/structure_score/fundamental_score` + 新 `sector_score`）。
2. **未來報酬**：`forward_return(price_rows, D, horizon)` = close(D+horizon 交易日)/close(D)−1，horizon 預設 20。資料不足（下市/停牌/缺）→ 該樣本剔除並計數。
3. **權重最佳化**：
   - 對權重單體（chip+struct+fund+sector=1）以粗網格（步長 0.1）列舉組合（約 286 組）。
   - 每組：對每個候選算加權分數 → 計 metric。
...
- 輸出 `backtest/report-<rundate>.md`（人看：建議權重＋metric＋穩定度＋樣本數＋警語）與 `backtest/report-<rundate>.json`（機器讀：完整結果）。
- 純函式盡量複用 `potential.py`；回測專屬邏輯放 `scripts/lib/backtest.py`。

## 檔案影響

- `scripts/lib/potential.py`（改）：新增 `sector_score`；`build_potential` 掛 `sector_s`；`finalize_scores` 納入 sector；`DEFAULTS` 加 `w_sector`（四項重新分配）。
...
- 測試：`scripts/tests/test_backtest.py`（純函式 TDD）、`scripts/tests/test_potential.py`（sector_score）。
