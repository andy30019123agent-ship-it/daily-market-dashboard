...
- **verdict**：**台美分列** `{tw:{...}, us:{...}}`，每個市場各含 `{stance, score, comment, bullish[], bearish[], risks[]}`：
  - `bullish/bearish/risks`：該市場的利多/利空/隱憂條列。
  - **`stance`**：該市場綜合多空總評（偏多 / 中性偏多 / 中性 / 中性偏空 / 偏空）。
  - **`score`**：0~100（0 極空、50 中性、100 極多）給多空儀表。
  - **`comment`**：1~2 句綜述，說明該市場為何下此判斷。
  - 台股看台股自身（指數/法人/類股/台指VIX）；美股看美股（指數/晶片/Fed/VIX）；分開判讀。
- **summary**：3~5 句，台美兩地重點，給 Telegram 用。

### 2b. 報告日期（重要）
**報告日期一律用 `partial['date']`（fetch 算出的「最新有資料的交易日」），不是日曆今天。**
台股若當天未開盤或資料尚未發布，`partial['date']` 會是前一交易日 → 直接沿用前一次收盤資料、覆寫同一天的檔案即可（不要硬生出一個帶舊數據的新日期）。寫檔與 update_index 都用 `partial['date']`。

### 3. 合併 + 驗證
```python
from scripts.merge_day import merge_day, update_index
from scripts.lib.schema import validate_day
rdate = partial["date"]                  # 最新交易日，非日曆今天
day = merge_day(partial, soft, rdate, updated_at="<台北 YYYY-MM-DD HH:MM>")
assert validate_day(day) == []          # 不通過就修，別上線壞資料
# 寫 public/data/<rdate>.json、update_index(rdate)
```
...
# 本機：用 telegram reply 工具發給 chat_id 542348223
# 雲端排程（無 telegram 外掛）：用 Telegram Bot API
#   curl -s "https://api.telegram.org/bot<TOKEN>/sendMessage" -d chat_id=542348223 --data-urlencode text="$text"
```
